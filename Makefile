...
startaws: ## Claims networks and clusters on AWS
	...
	@for i in $$(find demo/AWS/* -name "*.yaml" -type f | grep -vE "config|provider"); do kubectl apply -f $$i;done
...
startazure: ## Claims networks and clusters on AZURE
	...
	@for i in $$(find demo/Azure/* -name "*.yaml" -type f | grep -vE "config|provider"); do kubectl apply -f $$i;done
	@echo "Ressources successfully claimed!"
	@echo ""
.PHONY:startazure
...
startgcp: ## Claims networks and clusters on GCP
	...
	@for i in $$(find demo/GCP/* -name "*.yaml" -type f | grep -vE "config|provider"); do kubectl apply -f $$i;done
	@echo "Ressources successfully claimed!"
	@echo ""
.PHONY:startgcp
